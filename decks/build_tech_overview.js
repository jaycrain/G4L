...
// --- DOES THE TEXT ACTUALLY FIT ITS BOX? -------------------------------------------------------------------
// The floor check above catches text laid THROUGH the wordmark. It cannot catch text that overruns the box it
// was given, because pptx does not reflow or shrink — it just draws past the edge, and the .pptx looks fine to
// every tool except the one that matters (PowerPoint, opened by a human).
//
// That blind spot became load-bearing the moment we raised the type scale (Jay, 2026-09-01: "the font size is
// too small ... there's a ton of white space"). Bigger text in a fixed box is exactly how you turn a legibility
// fix into a layout regression, on slides nobody re-opens.
//
// THIS IS AN ESTIMATE AND IS TREATED AS ONE. It models Barlow at ~0.5em average advance, so it is accurate to
// within roughly a line — good enough to catch a paragraph overrunning by 40%, not good enough to adjudicate a
// box that is 3% tight. So it WARNS and never throws: a false alarm that fails the build would teach us to
// disable it, which is how a check dies. The number it prints is "estimated fill", and anything over 100% is a
// place to look, not a verdict.
const FIT = [];
// Which slide a warning belongs to. Without it the report names a text fragment and leaves you grepping for it,
// which is the difference between a check you act on and a check you skim.
let SLIDE = 0;
const slide = () => { SLIDE++; const sl = p.addSlide(); sl.background = { color: WHITE }; return sl; };
const emWidth = (bold) => (bold ? 0.53 : 0.50);
const estLines = (text, o) => {
  const parts = Array.isArray(text)
    ? text.map((r) => ({ t: String(r.text ?? ''), b: !!(r.options && r.options.bold) }))
    : [{ t: String(text ?? ''), b: !!o.bold }];
  const avail = (o.w ?? 0) - (o.margin === 0 ? 0 : 0.2);
  if (avail <= 0) return 99;
  // Walk the runs, wrapping as one continuous stream so rich text is measured the way it renders.
  let lines = 1, used = 0;
  for (const { t, b } of parts) {
    for (const seg of t.split('\n')) {
      const w = (seg.length * emWidth(b) * (o.fontSize ?? 12)) / 72;
      used += w;
      while (used > avail) { lines++; used -= avail; }
      if (t.includes('\n') && seg !== t.split('\n').slice(-1)[0]) { lines++; used = 0; }
    }
  }
  return lines;
};
const fit = (what, text, o) => {
  const lh = ((o.fontSize ?? 12) * 1.2 * (o.lineSpacingMultiple ?? 1)) / 72;
  const need = estLines(text, o) * lh;
  const have = o.h ?? 0;
  if (have > 0 && need > have) FIT.push({ slide: SLIDE, what, need, have, pct: Math.round((need / have) * 100), size: o.fontSize });
  return o;
};

const T  = (s, t, o) => s.addText(t, fit(typeof t === 'string' ? t.slice(0, 42) : 'rich', t, check('text', o)));
...
const chrome = (s, n) => {
  ...
  if (n) s.addText(String(n), { x:12.25, y:0.3, w:0.75, h:0.32, fontFace:HEAD, color:GREYL, fontSize:14, align:'right', margin:0 });
};
const title = (s, t, dek) => {
  T(s, t, { x:0.55, y:0.42, w:11.5, h:0.7, fontFace:HEAD, bold:true, color:NAVY, fontSize:32, align:'left', margin:0 });
  if (dek) T(s, dek, { x:0.57, y:1.12, w:11.8, h:0.4, fontFace:HEAD, color:GREY, fontSize:18, align:'left', margin:0 });
};
...
const band = (s, y, h, parts, fill) => {
  ...
  T(s, parts, { x:0.95, y, w:11.4, h, fontFace:HEAD, color:WHITE, fontSize:16.5, align:'left', valign:'middle', lineSpacingMultiple:1.12, margin:0 });
};
...
const cards = (s, y, h, items, accent) => {
  ...
  items.forEach((it, i) => {
    ...
    // THE HEAD RESERVES ONLY THE LINES IT USES. At 14.5pt every header happened to fit one line, so the fixed
    // 0.42" slot was enough and the constraint was invisible. At 18pt a few wrap ("A Companion that remembers",
    // slide 16) and a one-line slot prints the second line straight through the body beneath it.
    //
    // Reserving two lines for every card was the obvious fix and the wrong one: it took 0.22" of body height
    // from ~40 cards to buy headroom for three, and turned one overflow warning into four. So the height is
    // measured per card from the same estimator the fit check uses — one source for "how tall is this text",
    // rather than a layout rule and a checker that can disagree.
    const headOpts = { x:x+0.24, y:y+0.2, w:w-0.48, fontFace:HEAD, bold:true, color:it.color||accent||NAVY, fontSize:18, align:'left', valign:'top', margin:0 };
    const headH = Math.max(0.42, estLines(it.head, { ...headOpts, h:0.42 }) * 0.30);
    T(s, it.head, { ...headOpts, h:headH });
    const bodyY = y + 0.2 + headH + 0.06;
    T(s, it.body, { x:x+0.24, y:bodyY, w:w-0.48, h:(y + h - 0.24) - bodyY, fontFace:HEAD, color:BODY, fontSize:14, align:'left', valign:'top', lineSpacingMultiple:1.12, margin:0 });
  });
};
...
const numbered = (s, y, items) => {
  items.forEach((it, i) => {
    ...
    T(s, String(i + 1), { x:0.6, y:yy-0.005, w:0.34, h:0.34, fontFace:HEAD, bold:true, color:WHITE, fontSize:15, align:'center', valign:'middle', margin:0 });
    T(s, [{ text:it.head + '  ', options:{bold:true, color:NAVY} }, { text:it.body, options:{color:BODY} }],
      { x:1.1, y:yy-0.04, w:11.6, h:0.78, fontFace:HEAD, fontSize:15, align:'left', valign:'top', lineSpacingMultiple:1.1, margin:0 });
  });
};
...
const statGrid = (s, y, items) => {
  ...
  items.forEach((it, i) => {
    ...
    T(s, String(it[0]), { x, y:y+0.14, w, h:0.56, fontFace:HEAD, bold:true, color:it[2]||NAVY, fontSize:26, align:'center', margin:0 });
    T(s, it[1], { x:x+0.06, y:y+0.74, w:w-0.12, h:0.46, fontFace:HEAD, color:GREY, fontSize:12, align:'center', valign:'top', lineSpacingMultiple:1.05, margin:0 });
  });
};
...
const phaseRow = (s, y, name, sub, color, sessions) => {
  ...
  T(s, name, { x:0.82, y:y+0.06, w:1.55, h:0.34, fontFace:HEAD, bold:true, color, fontSize:15, align:'left', margin:0 });
  T(s, sub, { x:0.82, y:y+0.42, w:1.55, h:0.3, fontFace:HEAD, color:GREYL, fontSize:12, align:'left', margin:0 });
  ...
  items.forEach((label, i) => {
    ...
    T(s, label, { x:x+0.14, y, w:w-0.28, h:0.92, fontFace:HEAD, bold:true, color:WHITE, fontSize:12.5, align:'left', valign:'middle', lineSpacingMultiple:1.05, margin:0 });
  });
};
...
T(s, 'GRINTA FOR LIFE · THE BUILD, END TO END', { x:0.7, y:2.4, w:7.9, h:0.35, fontFace:HEAD, bold:true, color:TEAL, fontSize:15, charSpacing:2, align:'left', margin:0 });
T(s, 'Tech Overview', { x:0.66, y:2.78, w:8.0, h:1.1, fontFace:HEAD, bold:true, color:NAVY, fontSize:44, align:'left', margin:0 });
T(s, 'What we built — and how to begin.', { x:0.7, y:4.0, w:7.9, h:0.5, fontFace:HEAD, color:GREY, fontSize:23, align:'left', margin:0 });
T(s, `For the team   ·   ${F.version}   ·   ${F.date}   ·   Confidential`, { x:0.7, y:4.72, w:8.2, h:0.35, fontFace:HEAD, italic:true, color:GREYL, fontSize:14, align:'left', margin:0 });
chrome(s, null);
...
const step = (x, kicker, head, body, color) => {
  ...
  T(s, kicker, { x:x+0.26, y:2.08, w:3.2, h:0.3, fontFace:HEAD, bold:true, color, fontSize:13, charSpacing:1.4, margin:0 });
  T(s, head, { x:x+0.26, y:2.42, w:3.2, h:0.42, fontFace:HEAD, bold:true, color:NAVY, fontSize:19, margin:0 });
  T(s, body, { x:x+0.26, y:2.92, w:3.25, h:1.1, fontFace:HEAD, color:BODY, fontSize:14, valign:'top', lineSpacingMultiple:1.12, margin:0 });
};
...
s = slide();
title(s, `${F.memberSurfaces} surfaces a member lives in.`, 'Each one built, not sketched — and each says what it is for, in its own words.');
// THE SUBHEAD UNDER EACH NAME IS THE APP'S OWN (Jay, 2026-09-01). It is pulled from lib/content/panel-messaging.ts
// via measure.js, not retyped here: that file is the single source precisely because ten subpages hand-writing
// their own title was ten chances to drift, and a deck holding an eleventh copy would drift silently and in print.
// A wording change in the app reaches this slide by re-running measure.js.
const SURFACES = [
  ['Dashboard',    null],            // no rung in the ladder — it is not a panel, it is where the panels live
  ['Program',      F.panelSubs.program],
  ['Playbook',     F.panelSubs.playbook],
  ['ID Score',     F.panelSubs.idScore],
  ['Grinta Index', F.panelSubs.grinta],
  ['Badges',       F.panelSubs.badges],
  ['Reclaim List', F.panelSubs.reclaimList],
  ['Movement',     F.panelSubs.movement],
  ['Momentum',     F.panelSubs.momentum],
  ['Community',    F.panelSubs.community],
];
SURFACES.forEach(([label, sub], i) => {
  const c = i % 5, r = Math.floor(i / 5), w = 2.31, h = 1.82;
  const x = 0.6 + c * (w + 0.19), y = 1.72 + r * (h + 0.17);
  SH(s, rr, { x, y, w, h, rectRadius:0.12, fill:{color:OFF}, line:{color:LGREY, width:1} });
  T(s, label, { x:x+0.16, y:y+0.16, w:w-0.32, h:0.3, fontFace:HEAD, bold:true, color:NAVY, fontSize:16, align:'left', margin:0 });
  T(s, sub || 'Home — where the Companion and every panel live.',
    { x:x+0.16, y:y+0.52, w:w-0.32, h:h-0.68, fontFace:HEAD, color:sub ? BODY : GREYL, fontSize:11.5, align:'left', valign:'top', lineSpacingMultiple:1.14, margin:0 });
});
T(s, 'Every line above is the surface\u2019s own subhead, read from the app. Plus Account Settings \u2014 a subpage without a panel, where the app bends to their life.',
  { x:0.62, y:5.74, w:12.1, h:0.5, fontFace:HEAD, italic:true, color:GREY, fontSize:13, margin:0 });
chrome(s, 7);

// 8 · the Program as it runs today
s = slide();
title(s, 'The Program, as it runs today.', 'Cycle 1 · The Foundation — four phases, twelve Sessions, four Checkpoints.');
// EACH SESSION NOW SAYS WHAT IT IS, not just what it is called (Jay, 2026-09-01), carrying the detail from the
// program diagram. The turn counts are MEASURED, and only where a measurement exists: scripts/session-eval.ts
// plays a scripted member through the real engine against the live model, and six of the twelve Sessions have
// such a run. The other six carry no number rather than an estimate dressed as one.
...
const PROGRAM = [
  ...
  ['Rewire', 'the mind', TEAL, '63AFAF', [
    ['Disinformation Audit',    'Five domains. Catch the lies, write true lines.', TURNS['w1-disinformation']],
    ['Visualization Workshop',  'Build the day, piece by piece.',                  TURNS['w2-visualization']],
    ['False Start Protocol',    'Redirect · Reframe · Restart.',          TURNS['w3-false-start']],
  ], '6 items · Grinta read, ceremony, badge.'],
  ['Rebuild', 'the body', OLIVE, 'AFB35E', [
    ["What's Your Why?",        '12 items. Your reasons, not the shoulds.',        null],
    ['Strengths & Weaknesses',  '24 items across twelve skills.',                  TURNS['b2-strengths']],
    ['The Lifestyle Pilot',     'One movement habit, one eating habit.',           null],
  ], '6 items · Grinta read, ceremony, badge.'],
  ['Reclaim', 'the outcome', ORANGE, 'F18A63', [
    ['Looking Forward',         'Six passes over your Reclaim List.',              TURNS['c1-passes']],
    ['Bigger World Audit',      '20 items. Where your world widened.',             null],
    ['Quality Days',            'Name what makes a day good, then track them.',    null],
  ], '6 items · the capstone. A full cycle closed.'],
];
const ROW_H = 0.95, PITCH = 1.05;
PROGRAM.forEach(([name, sub, color, tint, sessions, cpBody], r) => {
  const y = 1.72 + r * PITCH;
  SH(s, rr, { x:0.6, y, w:0.09, h:ROW_H, rectRadius:0.04, fill:{color}, line:{width:0} });
  T(s, name, { x:0.82, y:y+0.14, w:1.55, h:0.3, fontFace:HEAD, bold:true, color, fontSize:14, align:'left', margin:0 });
  T(s, sub,  { x:0.82, y:y+0.46, w:1.55, h:0.28, fontFace:HEAD, color:GREYL, fontSize:11, align:'left', margin:0 });
  const items = [...sessions.map((v) => [...v, color]), ['Checkpoint', cpBody, null, tint]];
  const gap = 0.16, x0 = 2.5, w = (12.73 - x0 - gap * (items.length - 1)) / items.length;
  items.forEach(([label, body, turns, fill], i) => {
    const x = x0 + i * (w + gap);
    SH(s, rr, { x, y, w, h:ROW_H, rectRadius:0.1, fill:{color:fill}, line:{width:0} });
    T(s, label, { x:x+0.14, y:y+0.08, w:w-0.28, h:0.26, fontFace:HEAD, bold:true, color:WHITE, fontSize:12, align:'left', margin:0 });
    T(s, body,  { x:x+0.14, y:y+0.36, w:w-0.28, h:0.40, fontFace:HEAD, color:WHITE, fontSize:10.5, align:'left', valign:'top', lineSpacingMultiple:1.08, margin:0 });
    if (turns) T(s, `${turns} member turns, measured`, { x:x+0.14, y:y+0.76, w:w-0.28, h:0.18, fontFace:HEAD, bold:true, color:WHITE, fontSize:9.5, align:'left', margin:0 });
  });
});
// WHAT A SESSION COSTS A MEMBER, said honestly. The range is measured; the minutes are derived from it with a
// pace anchor taken from a real walk, and the slide says which is which rather than presenting both as findings.
T(s, [
  { text:`A Session ran ${F.sessionTurns.min}\u2013${F.sessionTurns.max} member turns`, options:{bold:true} },
  { text:` end-to-end against the live model \u2014 ${F.sessionTurns.measured} of ${F.guidedSessions} Sessions harnessed; the rest carry no number rather than a guess. At the pace one real walk ran, that is roughly ${F.minutesPerSession.low}\u2013${F.minutesPerSession.high} minutes. A Session has no timer: it saves as you go, and answering in depth takes longer.`, options:{} },
], { x:0.62, y:5.90, w:12.1, h:0.54, fontFace:HEAD, color:GREY, fontSize:12, valign:'top', lineSpacingMultiple:1.06, margin:0 });
chrome(s, 8);
...
T(s, 'Hardened August 2026: authorization on every server action · row-level isolation · logged operator access · explicit rules on who may read an intake transcript · rate-limited sign-in and single-use recovery tokens.',
  { x:0.62, y:5.9, w:12.1, h:0.5, fontFace:HEAD, italic:true, color:GREY, fontSize:14, valign:'top', lineSpacingMultiple:1.1, margin:0 });
chrome(s, 10);
...
[
  ...
  SH(s, rr, { x, y, w, h, rectRadius:0.12, fill:{color:OFF}, line:{color:LGREY, width:1} });
  T(s, head, { x:x+0.2, y:y+0.16, w:w-0.4, h:0.4, fontFace:HEAD, bold:true, color:NAVY, fontSize:15, margin:0 });
  T(s, body, { x:x+0.2, y:y+0.6, w:w-0.4, h:h-0.76, fontFace:HEAD, color:BODY, fontSize:12.5, valign:'top', lineSpacingMultiple:1.12, margin:0 });
});
T(s, "These aren't guidelines. They're tests — a violation stops the build. Every one was written after a real failure.",
  { x:0.62, y:5.72, w:12.1, h:0.55, fontFace:HEAD, bold:true, color:NAVY, fontSize:15, margin:0 });
chrome(s, 11);
...
T(s, 'One event exists only to record when the Companion lost context — a fault we would rather measure than assume away.',
  { x:0.62, y:5.28, w:12.1, h:0.4, fontFace:HEAD, italic:true, color:GREY, fontSize:14, margin:0 });
chrome(s, 12);
...
const panel = (x, kicker, big, unit, lines, color) => {
  ...
  T(s, kicker, { x:x+0.3, y:1.93, w:5.35, h:0.3, fontFace:HEAD, bold:true, color, fontSize:14, charSpacing:1.2, margin:0 });
  ...
  T(s, lines, { x:x+0.3, y:2.98, w:5.35, h:0.88, fontFace:HEAD, color:BODY, fontSize:14, valign:'top', lineSpacingMultiple:1.08, margin:0 });
};
...
T(s, `${F.activeDays} active days · ${F.workSessions} work sessions · median sitting ${F.medianHours} hours · longest ${F.longestHours}. The two figures are counted differently and overlap slightly, so the total is an estimate rather than a precise sum — and it is derived from its parts here, never typed.`,
  { x:0.62, y:5.02, w:12.1, h:1.2, fontFace:HEAD, color:BODY, fontSize:14, valign:'top', lineSpacingMultiple:1.1, margin:0 });
chrome(s, 13);
...
[
  ...
  T(s, v, { x:1.1, y, w:1.8, h:0.42, fontFace:HEAD, bold:true, color, fontSize:16.5, margin:0 });
  T(s, what, { x:3.0, y, w:9.7, h:0.42, fontFace:HEAD, color:BODY, fontSize:15, margin:0 });
});
chrome(s, 14);
...
T(s, 'Not a chatbot. Not a course.', { x:0.62, y:5.62, w:12.1, h:0.4, fontFace:HEAD, bold:true, color:NAVY, fontSize:16.5, align:'center', margin:0 });
chrome(s, 16);
...
], { x:0.62, y:1.7, w:12.1, h:1.15, fontFace:HEAD, color:BODY, fontSize:17, valign:'top', lineSpacingMultiple:1.2, margin:0 });
['The Fade  ·  the problem', 'The work  ·  the means', 'Your Comeback  ·  the outcome'].forEach((t, i) => {
  ...
  T(s, t, { x, y:3.05, w:3.85, h:0.72, fontFace:HEAD, bold:true, color:i===2?WHITE:NAVY, fontSize:14, align:'center', valign:'middle', margin:0 });
  if (i < 2) T(s, '→', { x:x+3.85, y:3.05, w:0.28, h:0.72, fontFace:HEAD, bold:true, color:GREYL, fontSize:19, align:'center', valign:'middle', margin:0 });
});
band(s, 4.15, 1.1, [
...
T(s, 'A safety net runs underneath — if things ever turn dark, the Companion connects you to real help right away, including the 988 Suicide & Crisis Lifeline.',
  { x:0.62, y:5.45, w:12.1, h:0.5, fontFace:HEAD, italic:true, color:GREY, fontSize:14, valign:'top', margin:0 });
chrome(s, 19);
...
T(s, [
  { text:'Open in your browser:   ', options:{fontSize:15} },
...
T(s, 'Grinta for Life is a Progressive Web App: it runs in any modern browser, and you can install it to your home screen for a full-screen, app-like experience with reminders. Sign up with your own email — each account is private to one person.',
  { x:0.62, y:2.85, w:12.1, h:0.85, fontFace:HEAD, color:BODY, fontSize:15, valign:'top', lineSpacingMultiple:1.15, margin:0 });
[['Desktop browser','The best place for Sessions.'],['iPad','Safari → Add to Home Screen.'],['iPhone','Safari → Add to Home Screen. Installing turns on reminders.'],['Android','Chrome → Install app.']]
  .forEach(([h, b], i) => {
    ...
    T(s, h, { x:x+0.2, y:4.06, w:w-0.4, h:0.36, fontFace:HEAD, bold:true, color:NAVY, fontSize:15, margin:0 });
    T(s, b, { x:x+0.2, y:4.46, w:w-0.4, h:0.8, fontFace:HEAD, color:BODY, fontSize:12.5, valign:'top', lineSpacingMultiple:1.1, margin:0 });
  });
chrome(s, 22);
...
T(s, "That's a full first sitting. Stopping here and coming back is fine.",
  { x:0.62, y:5.5, w:12.1, h:0.4, fontFace:HEAD, italic:true, color:GREY, fontSize:15, margin:0 });
chrome(s, 23);
...
], { x:0.62, y:1.8, w:8.75, h:2.7, fontFace:HEAD, color:BODY, fontSize:15, valign:'top', lineSpacingMultiple:1.18, margin:0 });
T(s, 'The outer ring is Reconnect — where every Comeback begins.',
  { x:9.45, y:4.7, w:3.1, h:0.4, fontFace:HEAD, italic:true, color:GREYL, fontSize:12, align:'center', margin:0 });
band(s, 5.2, 1.1, [
...
T(s, 'HOW FAR TO GO', { x:0.9, y:1.95, w:5.35, h:0.3, fontFace:HEAD, bold:true, color:NAVY, fontSize:13, charSpacing:1.4, margin:0 });
T(s, [
...
T(s, 'HOW TO ENGAGE WELL', { x:7.08, y:1.95, w:5.35, h:0.3, fontFace:HEAD, bold:true, color:TEAL, fontSize:13, charSpacing:1.4, margin:0 });
T(s, [
...
], { x:7.08, y:2.35, w:5.35, h:2.75, fontFace:HEAD, color:BODY, fontSize:14, valign:'top', lineSpacingMultiple:1.12, margin:0 });
T(s, 'A first Cycle runs about six weeks, for most people.',
  { x:0.62, y:5.45, w:12.1, h:0.4, fontFace:HEAD, bold:true, color:NAVY, fontSize:15, align:'center', margin:0 });
chrome(s, 25);
...
T(s, 'Note what helped or got in the way of being honest — that is the signal we most want.',
  { x:0.62, y:3.75, w:12.1, h:0.45, fontFace:HEAD, color:BODY, fontSize:16.5, align:'center', margin:0 });
T(s, 'Every fix in this deck came from someone walking the product and writing down what broke.',
  { x:0.62, y:4.4, w:12.1, h:0.45, fontFace:HEAD, italic:true, color:GREY, fontSize:14, align:'center', margin:0 });
chrome(s, 26);
...
T(s, 'Take your time. We built it to meet you honestly — meet it the same way.',
  { x:0.6, y:5.5, w:12.13, h:0.5, fontFace:HEAD, color:GREY, fontSize:18, align:'center', margin:0 });
chrome(s, null);

const OUT = path.join(process.env.HOME, 'Desktop', 'G4L-Tech-Overview.pptx');
p.writeFile({ fileName: OUT }).then(() => {
  console.log(`WROTE ${OUT}  ·  27 slides · ${F.version} · floor ${FLOOR} enforced`);
  if (!FIT.length) { console.log('fit: no boxes estimated over 100%'); return; }
  FIT.sort((a, b) => b.pct - a.pct);
  console.log(`\nfit: ${FIT.length} box(es) estimated over their height (estimate ±1 line — look, don't obey):`);
  for (const f of FIT) console.log(`  s${String(f.slide).padStart(2)}  ${String(f.pct).padStart(4)}%  ${f.size}pt  need ${f.need.toFixed(2)}" have ${f.have.toFixed(2)}"  ${f.what}`);
});
