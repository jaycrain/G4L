// G4L — "Beginning Your Comeback", the member/team tutorial.
//
// COWORK'S GENERATOR, updated by CC on 2026-08-30 for what actually shipped. Her chrome, layout and voice are
// untouched; only the slides describing PROGRAM STRUCTURE changed, because that structure changed under them on
// 28 August and the July deck now teaches the wrong shape:
//
//   · Reconnect was ONE 65-minute session. It is four Sessions now, and the FIRST is the Mirror (the ID Score),
//     not the Doors. The July deck sends a member to the Doors first — literally the wrong door.
//   · The Doors is R2 now, and it is called EXCAVATION (Jay, 2026-08-28).
//   · The Tour is re-run from the Account page. The Field Guide it used to live on was retired on 8 August and
//     now redirects — an instruction that would have sent a member to a page that no longer exists.
//
// Everything else is hers, verbatim. Edit this script, never the .pptx.
...
s.addText("August 30, 2026   ·   For our team and founding Charter members   ·   Confidential",{x:0.7,y:5.05,w:7.8,h:0.35,fontFace:HEAD,italic:true,color:GREYL,fontSize:13,align:"left",margin:0});
...
s=p.addSlide();s.background={color:WHITE};
title(s,"Your first Sessions.","Reconnect is four short sittings — and the first one measures where you stand.");
s.addText([{text:"Reconnect opens with the ",options:{}},{text:"Mirror",options:{bold:true}},{text:" \u2014 the questions behind your ID Score. It is the baseline everything later is measured against, so answer it as you actually are, not as you mean to be.",options:{}},{text:"\n\nThen comes ",options:{}},{text:"Excavation",options:{bold:true}},{text:" \u2014 the Doors. You will go deeper on the ones your Fade came through, and most of us walked through more than one: a career that ended, a body that started saying no, a marriage that drifted, a loss that changed everything.",options:{}}],{x:0.6,y:1.95,w:7.5,h:2.35,fontFace:HEAD,color:BODY,fontSize:16.5,align:"left",valign:"top",lineSpacingMultiple:1.14,margin:0});
const rsess=[["R1","Mirror",TEAL],["R2","Excavation",TEAL],["R3","Drift + Letter",TEAL],["R4","Checkpoint",OLIVE]];
rsess.forEach((r,i)=>{const x=0.6+i*1.9;s.addShape(rr,{x,y:4.45,w:1.75,h:0.78,rectRadius:0.1,fill:{color:r[2]},line:{width:0}});s.addText([{text:r[0],options:{bold:true,fontSize:12,breakLine:true}},{text:r[1],options:{fontSize:10.5}}],{x,y:4.45,w:1.75,h:0.78,fontFace:HEAD,color:WHITE,align:"center",valign:"middle",lineSpacingMultiple:1.0,margin:0});});
s.addShape(rr,{x:0.6,y:5.45,w:7.5,h:1.1,rectRadius:0.12,fill:{color:NAVY},line:{width:0}});
s.addText([{text:"Answer fully.  ",options:{bold:true}},{text:"This is where the Companion begins to know you. A sentence more beats a sentence less.",options:{}}],{x:0.9,y:5.45,w:6.9,h:1.1,fontFace:HEAD,color:WHITE,fontSize:15.5,align:"left",valign:"middle",lineSpacingMultiple:1.1,margin:0});
...
p.writeFile({fileName:require("path").join(process.env.HOME,"Desktop","G4L_Beginning_Your_Comeback_Tutorial.pptx")}).then(f=>console.log("WROTE",f)).catch(e=>{console.error(e);process.exit(1);});
