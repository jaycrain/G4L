...
test('Rewire W1 e2e — the REAL drain + state round-trip lands EVERY true line as a Playbook keeper', async () => {
  ...
  for (const line of lines) {
    ...
    await drainHarvest(db, memberId, prev, t.state, 'rewire'); // the REAL drain the action runs
  ...
  assert.equal(rows.length, 2, `both true lines should land as keepers (got ${rows.length}) — 0/1 reproduces the prod drop`);
