...
import { RHYTHM_WEEKLY_MAX, BACKOFF_AFTER_IGNORED, BACKOFF_FLOOR, DEFAULT_QUIET, type Rhythm } from './config.ts';
...
export function cadenceCheck(i: CadenceInput): CadenceResult {
  ...
  // In-app doesn't count against the ceiling and isn't a reach-out, so it skips the ceiling + quiet hours.
  if (i.channel === 'in_app') return { ok: true, reason: null, effectiveRhythm: eff };
