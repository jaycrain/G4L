...
export function bindGoalItem(beat: Beat, items: ReclaimItem[]): ReclaimItem | null {
  ...
  const candidates = items.filter(
    ...
    // "any" one), so no goal close (and no fog close) ever fires for them. They advance via the
    // companion mark instead. See docs/reclaim-anygoal.md.
    (i) => isOpen(i) && isCoachedCategory(i.category) && !isVagueReclaim(i.text) && (cat === 'any' || i.category === cat),
  );
