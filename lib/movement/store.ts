...
export async function logMovement(
  ...
  // Defaults to the MEMBER'S today, not the server's. This used to be `coalesce($5::date, current_date)` — i.e.
  // UTC — which put an evening activity in Boulder on tomorrow's date.
  const on = input.occurredOn && /^\d{4}-\d{2}-\d{2}$/.test(input.occurredOn)
    ? input.occurredOn
    : await memberToday(db, memberId);
  await db.query(
...
export async function listMovementLog(db: Db, memberId: string, days = 30): Promise<MovementLogEntry[]> {
  const { rows } = await db.query<{ id: string; source: MovementLogSource; activity_type: MovementKind; note: string | null; occurred_on: string; days_ago: number }>(
    `select id, source, activity_type, note,
            ...
            ($3::date - occurred_on)::int as days_ago
       from movement_log
      where member_id = $1 and occurred_on >= ($3::date - ($2 || ' days')::interval)
      order by occurred_on desc, created_at desc`,
    // days_ago and the window are both measured from the MEMBER'S today — "3 days ago" should mean three of their
    // days, not three UTC days that end at 6pm where they live.
