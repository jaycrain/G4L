...
| **v2.0** | **Staged capture** — authoritative stage machine (`identity → gap → reclaim → card`), per-field tools (model *tags* each piece, no guess), confirmed transitions, stage-scoped capture, lighter Door posture, front-loader parking. Reverses v1's order (ends on hope). | `onboarding-v2.0` *(at cut-over)* | target **≥87%** | The rewrite. Shape: `docs/onboarding-staged-capture-shape.md`. Built behind a flag, slices a→d, cut over only when the eval clears the bar. |
