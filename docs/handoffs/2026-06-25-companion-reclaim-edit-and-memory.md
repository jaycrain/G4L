...
- *Governance note on the audit rail — follow-up TAKEN (Cowork's call, 2026-06-25):* the soft-delete already gave "nothing destroyed, reversible." On top of that, **migration `0041` consolidates the trail into ONE log**: a `source` column on `member_profile_audit` (member_profile rows default to 'member_profile', existing trigger untouched) + a DB **trigger on `reclaim_item`** (source='reclaim_item') that logs every add / remove / reorder / refine / mark — `removed_at` lands as an UPDATE row, the volatile `last_served_at` is excluded so it can't flood the log. Now a single query of `member_profile_audit` for a member shows all their member-data mutations. Verified against PGlite (adds → `_created`, remove → `removed_at`, no `last_served_at` rows, all source='reclaim_item').
